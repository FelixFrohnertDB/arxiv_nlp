...
class MLP(nn.Module):
    def __init__(self, raw_size, drop=0.25):
        super().__init__()

        self.raw = nn.Sequential(
            Flatten(),
            nn.Linear(raw_size, 128),
            nn.PReLU(),
            nn.BatchNorm1d(128),
            nn.Dropout(drop),
            nn.Linear(128, 64),
...
class MlpDepth(nn.Module):
    def __init__(self, raw_size, depth, drop=0.25):
        ...
        for _ in range(depth):
            output_size = max(
                1, input_size // 2
            )  # Ensure the output size does not go below 1
            layers.extend(
                [
        ...
        # Final output layer
        layers.extend([nn.Linear(input_size, 1), nn.Sigmoid()])
...
def train_model(
    ...
    num_epochs=50,
    patience=7,
    file_name="saved_files/best_x_model.pth",
):
