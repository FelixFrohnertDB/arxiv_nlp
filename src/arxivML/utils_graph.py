...
def create_training_data(
    ...
    for yy in years:
        ...
        G_curr = nx.from_scipy_sparse_array(
            adj_mat_sparse_curr, parallel_edges=False, create_using=nx.MultiGraph
        )
    ...
    vertex_large_degs = all_vertices[
        all_degs >= vertex_degree_cutoff
    ]  # use only vertices with degrees larger than 10.
    print("\n")
...
def create_training_data_biased(
    full_graph,
    year_start,
    years_delta,
    NUM_OF_VERTICES,
    ...
    while (cT < (edges_used / 2)) or (cF < (edges_used / 2)):
        ...
        if v1 != v2 and not all_G[0].has_edge(v1, v2):  ## unconnected in 2018
            ...
            if pair not in added_pairs:
                ...
                if (
                    len(unconnected_vertex_pairs) % 10**4 == 0
                    and len(unconnected_vertex_pairs) != old_c
                ):
                    time_end = time.time()

                    print(
                        f"    edge progress ({time_end-time_start}sec): {len(unconnected_vertex_pairs)/10**6}M/{edges_used/10**6}M; True: {cT}; False: {cF}"
                    )
                    ...
                    with open(data_source + "_logs.txt", "a") as myfile:
                        myfile.write(
                            ...
                            + "sec): "
                            + str(len(unconnected_vertex_pairs) / 10**6)
                            + "M/"
                            + str(edges_used / 10**6)
                            + "M "
                            + str(cT)
...
def calculate_ROC(
    data_vertex_pairs, data_solution, train_info="", save=False, directory=""
):
...
def train_model(
        ...
        with torch.no_grad():
            ...
            if iteration % 50 == 0:
                info_str = (
                    ...
                    + str(iteration)
                    + " - train loss: "
                    + str(np.mean(all_real_loss[0]) + np.mean(all_real_loss[1]))
                    + "; test loss: "
                    + str(np.mean(all_real_loss[2]) + np.mean(all_real_loss[3]))
                )
            ...
            if test_loss_total[-1] == min(test_loss_total):  ## store the best model
                ...
                torch.save(
                    model_semnet,
                    f"saved_ml_models/baseline_3_full_{str(year_start)}_{str(year_delta)}",
                )
                torch.save(
                    model_semnet.state_dict(),
                    f"saved_ml_models/baseline_3_state_{str(year_start)}_{str(year_delta)}.pt",
                )
                model_semnet.train()
                with open(data_source + "_logs.txt", "a") as myfile:
                    myfile.write("\n    stored model ")
            ...
            if len(test_loss_total) > 200:  # early stopping
                ...
                if len(moving_avg) > 10:
                    ...
                    if (
                        moving_avg[-1] > moving_avg[-5]
                        and moving_avg[-1] > moving_avg[-25]
                    ):
    ...
    plt.title(
        f"test loss: year_start: {str(year_start)}, year_delta: {str(year_delta)}"
    )
    plt.show()

    plt.plot(test_loss_total[500:])
    plt.title(
        f"test loss, last 500: year_start: {str(year_start)}, year_delta: {str(year_delta)}"
    )
    plt.show()

    plt.plot(moving_avg)
    plt.title(
        f"moving avg loss: year_start: {str(year_start)}, year_delta: {str(year_delta)}"
    )
    plt.show()
    plt.close()
...
def compute_all_properties_of_list(all_sparse, vlist, data_source):
    ...
    for ii in range(len(vlist)):
        ...
        if ii % 10**4 == 0:
            print(
                "    compute_all_properties_of_list progress: (",
