...
def get_co_occur_concept_pair_after_year_arr(
    word_co_occurrences: dict, first_occ_year: int, final_occ_year: int
) -> np.ndarray:
...
def get_years_range(year_arr: np.ndarray, start: int, end: int) -> np.ndarray:
    return np.unique(year_arr)[start:] if end == -0 else np.unique(year_arr)[start:end]


def get_baseline_1_embeddings(
    loaded_w2v: KeyedVectors, filtered_concept_arr: np.ndarray, embedding_dim: int = 128
):
    """
...
def load_model_for_year(year, model_type):
    ...
    try:
        ...
        raise ValueError(
            f"Invalid model type: {model_type}. Choose from 'time_informed_simple', 'time_informed_retrain', or 'no_time_informed'."
        )


def get_method_embeddings(
    filtered_concept_arr: np.ndarray,
    year_arr: np.ndarray,
    model_type: str,
    ...
    if load:
        ...
        c_inx_arr = np.memmap(
            "saved_files/embedding_concept_arr.dat", shape=(10235,), dtype="<U55"
        )
        c_encoding_arr = np.memmap(
            "saved_files/embedding_vector_arr.dat",
            shape=(10235, 30, 128),
            dtype=np.float64,
        )
    else:
        ...
        for cnt, concept in enumerate(
            tqdm(concept_inx_arr, desc="Preparing encoding array")
        ):
            year_vectors = c_dict[concept]
            c_encoding_arr[cnt] = np.array(
                [
                    year_vectors.get(year, np.zeros(embedding_dim))
                    for year in unique_years
                ]
            )
        ...
        if save:
            ...
            c_inx_arr = np.memmap(
                "saved_files/embedding_concept_arr.dat",
                dtype="<U55",
                mode="r",
                shape=c_inx_arr.shape,
            )
            c_encoding_arr = np.memmap(
                "saved_files/embedding_vector_arr.dat",
                dtype=np.float64,
                mode="r",
                shape=c_encoding_arr.shape,
            )
...
def compute_acc(model, dataloader):
    ...
    with torch.no_grad():
        for data, labels, _, _ in dataloader:
            ...
            predicted = (
                outputs > 0.5
            ).float()  # Use 0.5 as threshold for binary classification
